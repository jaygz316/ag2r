...
- **Two execution contexts.** AG2.0 Electron exposes default + isolated contexts. `server.js` locks `preferredContextId` to the default context to prevent hash oscillation. Any transient error in the default context can fall back to the isolated context, but it must not lock there permanently. Furthermore, all interactive actions (clicks, typing, stop button, settings) must be executed with `{ forceDefaultContext: true }` in the default context, as isolated contexts do not have access to the page's React event handlers.
...
- **Sticky user prompts.** User's last prompt has `position: sticky` in AG2.0's CSS. The capture script marks these with `data-ag-sticky` and forces `backgroundColor: var(--bg-app)` on the clone. AG2R does NOT override sticky — AG2.0's own CSS handles it.
