...
// Targets element by placeholder text or by clickId (sched:N / scheddlg:N) within the dialog overlay (z-[5000] / z-[2550]).
...
app.post('/type-text', async (req, res) => {
  ...
  (() => {
    ...
    // Strategy 1: find by placeholder (updated to support both z-[5000] and z-[2550] overlays)
    ...
    if (!el) {
      ...
      if (clickId) {
        ...
        if (prefix === 'sched') {
          ...
          // Updated to query z-[5000] and z-[2550] overlays
          const overlay = document.querySelector('.fixed.inset-0[class*="z-[5000]"], .fixed.inset-0[class*="z-[2550]"]');
          if (overlay) {
